...
const CreateGroup = () => {
  ...
  const addGroup = () => {
    ...
    setTimeout(() => {
      error ? setSuccess("") : setSuccess("Password changed successfully");
      setLoading(false);
    }, 2000);
  ...
  return (
    <div className="Create-Group-Container">
      <div className="Create-Group-Container-Header">
        ...
        {error && <Alert severity="error">Creating group faild!</Alert>}
        <TextField
          label="Group name"
          variant="outlined"
      ...
      <div className="Create-Group-Container-Main">
        {users.map((elem) => {
          if (elem?.role?.name === "Student") return UserJSX(elem);
        })}
      </div>
