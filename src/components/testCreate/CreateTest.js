...
function TestCreater() {
  ...
  const addQuestionRow = (e) => {
    ...
    newElement.querySelector('input[type=radio]').name = `Question${questionNumber - 1}`
  ...
  const addTest = (e) => {
    ...
    const testSubject = document.querySelector('[data-value="testSubject"]').value
    ...
    const testRigthAnswers = document.querySelectorAll('[data-value="rigthAnswer"]')
    ...
    testAnswers.forEach(element => {
      const questionNumber = +element.parentElement.childNodes[0].name.split('Question')[1];
      if (isNaN(questionNumber)) return
      else if (typeof answers[questionNumber] !== 'object') answers[questionNumber] = []
        answers[questionNumber].push(element.value)
    })

    testQuestions.forEach((element, index) => {if (index) questions.push(element.value)})

    testRigthAnswers.forEach(element => {if (element.checked) rigthAnswers.push(+element.value - 1)})
          ...
          <div>
            ...
            <div className="AnswersForm">
              <div className="Answer">
                <input type='radio' name="Question1Answer" data-value='rigthAnswer'/>
