...
import { fetchUsersSuccess, fetchUsersPending, fetchUsersFailure, createUserSuccess, updateUserSuccess, deleteUserSuccess } from "./actions";
...
const usersReducer = createReducer(initialState, builder => {
    builder
        ...
        .addCase(deleteUserSuccess, (state, { payload }) => {
            state.users = state.users.filter(user => user.id !== payload.deletedUser.id);
        })
        // FETCH PENDING AND FAILURE
        .addCase(fetchUsersPending, (state, { payload }) => {
            state.loading = true;
            state.error = null;
        })
